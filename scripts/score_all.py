...
class Score:
    ...
    def cs_report(self):
        ...
        report_sum = classification_report(y_flat, y_pred_falt, digits=4)
        print(classification_report(y_flat, y_pred_falt, digits=4))
        return y_pred_argmax, report, report_sum
    ...
    def calc_scores(self):
        ...
        y_pred_max, report, report_sum = self.cs_report()
        IOU_keras = self.mean_iou(y_pred_max)
        self.iou_classes(IOU_keras)
        # self.accuracy(report)
        return report, IOU_keras.result().numpy(), report_sum
